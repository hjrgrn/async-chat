...
use crate::shared_lib::socket_handling::{RecvHandler, WriteHandler, HMAC_KEY_SIZE};
use crate::{
    client_lib::globals::CLIENT_COM,
    globals::CONNECTION_ACCEPTED,
    shared_lib::{OutputMsg, StdinRequest},
};

/// TODO: comment, custom error, error handling.
/// TODO: we could have a client that automatically retries if something went
...
pub async fn handshake(
    ...
    tokio::select! {
        ...
        r = read_handler.recv_str(&mut response) => {
            r?;
            handles_response(output_tx, &response).await?;
        }
    }
...
///
/// Handles the server's acceptance message. If Ok is returned, the client is been accepted,
    ...
    if response == CONNECTION_ACCEPTED {
        output_tx
            .send(OutputMsg::new(&format!(
                "You have been accepted, type \"{}\" for displaying all the avaible commands",
                CLIENT_COM.trim()
            )))
            .await?;
        return Ok(());
    } else {
        let msg = "Connection refused due to: {response}.";
